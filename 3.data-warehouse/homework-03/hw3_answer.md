...
SELECT COUNT(*) FROM `de-zoomcamp-486100.nytaxi.materialized_yellow_tripdata_2024` 
 ```
- Answer: 20,332,093 total records of Yellow Taxi Data
...
- Answer:BigQuery is a columnar database and only scans the columns requested in a query.  
(Source: [BigQuery Columnar Storage](https://cloud.google.com/bigquery/docs/storage-model))
...
- Answer: False,Clustering should be applied only when it improves query performance. It is not required for every table (Source:[BigQuery Partitioning and Clustering](https://cloud.google.com/bigquery/docs/partitioned-tables)).


## Question 9. Understanding table scans

No Points: Write a `SELECT count(*)` query FROM the materialized table you created. How many bytes does it estimate will be read? Why?
- Answer: 0 byte ,BigQuery computes COUNT(*) using table metadata for materialized tables. No column data is scanned, resulting in zero bytes processed.
